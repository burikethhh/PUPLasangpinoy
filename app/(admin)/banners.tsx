...
export default function AdminBanners() {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        <ActivityIndicator size="large" color="#F25C05" style={{marginTop: 50}} />
      ) : (
        <FlatList
          data={banners}
          keyExtractor={(i) => i.id}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => setModalVisible(false)}><Ionicons name="close" size={28} color="#555" /></TouchableOpacity>
            </View>
            
            <View style={styles.formGroup}>
              <Text style={styles.label}>Banner Name (Offer/Title)</Text>
              <TextInput style={styles.input} value={form.name} onChangeText={(t) => setForm({...form, name: t})} placeholder="e.g. Summer Sale!" />
            </View>

            <View style={styles.formGroup}>
              <Text style={styles.label}>Image URL</Text>
              <TextInput style={styles.input} value={form.imageUrl} onChangeText={(t) => setForm({...form, imageUrl: t})} placeholder="https://..." />
            </View>
            ...
            <View style={[styles.formGroup, { flexDirection: "row", alignItems: "center", justifyContent: "space-between" }]}>
              ...
              <Switch value={form.active} onValueChange={(v) => setForm({...form, active: v})} trackColor={{ true: "#F25C05" }} />
            </View>

            <TouchableOpacity style={[styles.saveBtn, saving && {opacity: 0.7}]} onPress={handleSave} disabled={saving}>
              {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveTxt}>Save Banner</Text>}
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", padding: 15, backgroundColor: "#fff", borderBottomWidth: 1, borderColor: "#e1e4e8" },
  ...
  cardInfo: { padding: 15, flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", alignItems: "center" },
  cardTitle: { fontSize: 16, fontWeight: "bold", width: "60%" },
  cardStatus: { fontSize: 14, color: "#888", width: "20%" },
  cardActions: { flexDirection: "row", gap: 15, width: "20%", justifyContent: "flex-end" },
  iconBtn: { padding: 4 },
  ...
  saveBtn: { backgroundColor: "#F25C05", padding: 15, borderRadius: 8, alignItems: "center", marginTop: 10 },
